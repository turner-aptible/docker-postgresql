FROM quay.io/aptible/debian:bullseye
...
RUN set -x \
 ...
 && ICU_TEMP_DEB="$(mktemp)" \
 && wget -O "$ICU_TEMP_DEB" "https://snapshot.debian.org/archive/debian/20180101T053014Z/pool/main/g/glibc/locales-all_2.25-6_amd64.deb" \
 && dpkg -i "$ICU_TEMP_DEB" \
 && rm -f "$ICU_TEMP_DEB" \
 #&& apt-get purge -y libicu* \
 #&& apt-get update && apt-get install -y --no-install-recommends apt-transport-https ca-certificates wget gnupg2 && rm -rf /var/lib/apt/lists/* \
 #&& ICU_TEMP_DEB="$(mktemp)" \
 #&& wget -O "$ICU_TEMP_DEB" "http://ftp.de.debian.org/debian/pool/main/i/icu/libicu67_67.1-7_amd64.deb" \
 #&& dpkg -i "$ICU_TEMP_DEB" \
 #&& rm -f "$ICU_TEMP_DEB" \
...
RUN sed -ri  's/__DEBIAN__VERSION__/bullseye/' /etc/apt/sources.list.d/pgdg.list
RUN sed -i 's/apt-archive/apt/g' /etc/apt/sources.list.d/pgdg.list
...
ENV AUTH_METHOD peer
ENV PRELOAD_LIB "pg_stat_statements,pglogical"
ENV TAG 15
